      ...
      ASTStatement::Extern(extern_stmt) => {
        ...
        for item in &extern_stmt.items {
          ...
          match item_node {
            ...
            ASTNode::Statement(ASTStatement::Constant(const_)) if const_.value.is_some() => {
            ...
            ASTNode::Statement(ASTStatement::Variable(var)) if var.value.is_some() => {
  ...
  fn extra_checks_expression(
    ...
    match expr {
      ...
      ASTExpression::Lambda(lambda) => {
        ...
        // Define lambda parameters in scope so the body's undeclared-variable check passes.
